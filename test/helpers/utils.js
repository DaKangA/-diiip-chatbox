...
import Bus from 'vue/dist/vue.js'
...
Vue.prototype.$$ = function $$(selector) {
  ...
  const fn = vmEls.length ? el => vmEls.find(el) : el => this.$el === el
  const found = Array.from(els).filter(fn)
  return found.length ? found : emptyNodes
}

Vue.prototype.$ = function $(selector) {
  const els = document.querySelectorAll(selector)
  const vmEl = this.$el.querySelector(selector)
  const fn = vmEl ? el => el === vmEl : el => el === this.$el
